...
export default function CustomerView({ ridOverride } = {}) {
  ...
  const isOpen = useMemo(() => {
    ...
    if (!horarios) return manualIsOpen;
    const days = ['domingo','segunda','terca','quarta','quinta','sexta','sabado'];
    const today = days[new Date().getDay()];
    const s = horarios[today];
    // Dia marcado como inativo nos horários → só abre se admin forçou (is_open=true)
    if (!s?.ativo) return manualIsOpen === true;
    // Dia ativo: horários são a fonte primária. is_open=false = fechamento de emergência.
    if (manualIsOpen === false) return false;
    ...
    try {
      if (isPublic) {
        ...
        if (result.success) {
          ...
          setOrderResult({ codigo: result.codigo || '', whatsapp: result.whatsapp || null });
          setStep('success');
        } else {
  ...
  if (step === 'success') {
    const waNumber = orderResult?.whatsapp?.replace(/\D/g, '') || '';
    const waMsg = encodeURIComponent(`Olá! Gostaria de acompanhar meu pedido *${orderResult?.codigo || ''}* 😊`);
    ...
    return (
      <div className="customer-wrapper success-step" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 400, width: '100%' }}>
          <div style={{ fontSize: '5rem', marginBottom: 16, animation: 'successPop 0.5s cubic-bezier(0.16,1,0.3,1)' }}>🎉</div>
          <h2 style={{ fontSize: '1.6rem', fontWeight: 900, marginBottom: 8, letterSpacing: '-0.03em' }}>Pedido confirmado!</h2>
          {orderResult?.codigo && (
            <div style={{ display: 'inline-block', background: '#f0fdf4', color: '#16a34a', border: '1.5px solid #bbf7d0', borderRadius: 10, padding: '6px 18px', fontWeight: 800, fontSize: '1.1rem', marginBottom: 16, letterSpacing: '0.02em' }}>
              {orderResult.codigo}
            </div>
          )}
          <p style={{ color: '#666', lineHeight: 1.6, marginBottom: 4 }}>Recebemos seu pedido! Em breve entraremos em contato.</p>
          <p style={{ color: '#AAA', fontSize: '0.85rem', marginBottom: 28 }}>⏱ Previsão: {deliveryTime}</p>
          ...
          {waUrl && (
            <a
              ...
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, width: '100%', padding: '16px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 16, fontWeight: 800, fontSize: '1rem', cursor: 'pointer', fontFamily: 'inherit', textDecoration: 'none', marginBottom: 12, boxSizing: 'border-box' }}
            >
          ...
          <button
            ...
            style={{ width: '100%', padding: '14px', background: waUrl ? '#f5f5f5' : primaryColor, color: waUrl ? '#555' : '#fff', border: 'none', borderRadius: 16, fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', fontFamily: 'inherit' }}
          >
            Fazer outro pedido
          </button>
        </div>
        ...
        <div className="checkout-revamp">
          <div className="revamp-section">
            ...
            <div className="checkout-items-list">
              {cart.map((item, idx) => {
                const detail = [item.variation, ...(Array.isArray(item.complements) ? item.complements : [])].filter(Boolean).join(' · ');
                return (
                  <div key={idx} className="checkout-item-row">
                    <span className="checkout-item-qty">{item.qty}x</span>
                    <span className="checkout-item-name">
                      {item.name}
                      {detail && <div className="checkout-item-detail">{detail}</div>}
                    </span>
